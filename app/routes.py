...
from app import app
from flask import request, jsonify, Response # <-- Add Response here
...
def upload_leads():
    ...
    if file and file.filename.endswith('.csv'):
        try:
            ...
            print(f"Leads CSV uploaded with {len(datastore.leads_df)} rows.")
            return jsonify({
...
def download_results():
    ...
    if datastore.leads_df is None or 'score' not in datastore.leads_df.columns:
        return jsonify({"error": "No scored results found to download."}), 404

    # Convert DataFrame to CSV format in memory
    csv_data = datastore.leads_df.to_csv(index=False)
